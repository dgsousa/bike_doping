...
(function () {
	...
	var canvas = d3.select(".scatter_plot").append("svg").attr("width", width + padding * 2).attr("height", height + padding * 2).append("g").attr("transform", "translate(" + padding + "," + padding + ")");

	var header = d3.select("svg").append("text").text("Doping in Professional Bicycle Racing").attr("font-size", "36px").attr("stroke", "black").attr("dx", 320).attr("dy", 60);
	...
	var xAxis = d3.axisBottom(xScale).ticks(4).tickFormat(function (num) {
	...
	d3.json("https://raw.githubusercontent.com/FreeCodeCamp/ProjectReferenceData/master/cyclist-data.json", function (err, data) {
		if (err) console.log(err);else {
			...
			var dots = canvas.selectAll("g").data(data).enter().append("g").attr("transform", function (d) {
				var x = xScale(d.Seconds - minTime);
				var y = yScale(d.Place);
				return "translate(" + x + "," + y + ")";
			}).style("fill", function (d) {
				return d.Doping ? "red" : "grey";
			}).on("mouseover", function (d) {
				return tooltip.style("opacity", .8).style("transition", "opacity .3s ease-in").html("<h3>" + d.Name + ": " + d.Nationality + "</h3><br>" + "<h3>Year: " + d.Year + ", " + "Time: " + d.Time + "</h3><br><h3>" + d.Doping + "</h3>");
			}).on("mouseout", function () {
				return tooltip.style("opacity", 0).style("transition", "opacity .3s ease-in");
			...
			dots.append("text").text(function (d) {
				return d.Name;
			}).attr("text-anchor", "end").attr("font-size", "1em").attr("fill", "black").attr("dx", -20);

			var key = canvas.append("g").attr("transform", "translate(800, 500)");

			key.append("circle").attr("r", 10).attr("fill", "grey");

			key.append("text").text("No doping allegations").attr("font-size", "18px").attr("fill", "black").attr("dx", 20).attr("dy", 5);

			key.append("circle").attr("r", 10).attr("fill", "red").attr("cy", 50);

			key.append("text").text("Riders with doping allegations").attr("font-size", "18px").attr("fill", "black").attr("dx", 20).attr("dy", 55);

			canvas.selectAll("circle").on("mouseover", function () {
				d3.select(this).attr("stroke", "black");
			}).on("mouseout", function () {
				d3.select(this).attr("stroke", null);
			});

			canvas.append("g").call(yAxis).append("text").text("Ranking").attr("text-anchor", "end").attr("font-size", "22px").attr("stroke", "black").attr("fill", "black").attr("transform", "rotate(-90)").attr("dy", -40).attr("dx", -300);

			canvas.append("g").call(xAxis).attr("transform", "translate(0," + height + ")").append("text").text("Minutes behind fastest time").attr("font-size", "22px").attr("stroke", "black").attr("fill", "black").attr("dy", 50).attr("dx", 500);

			var tooltip = d3.select(".tooltip").append("div").style("width", "250px").style("height", "150px").style("position", "absolute").style("top", "250px").style("left", "400px").style("z-index", 10).style("background", "black").style("opacity", 0);
		}
	});
})();
